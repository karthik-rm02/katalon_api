...
def petPayload = new HttpTextBodyContent(replaceSuffixWithUUID('{"category": {"id": 1, "name": "Test Category"}}'))
petRequest.setBodyContent(petPayload)
addHeaderConfiguration(petRequest)
def petResponse = WSBuiltInKeywords.sendRequest(petRequest)
WSBuiltInKeywords.verifyResponseStatusCode(petResponse, 200)

def verifyRequest = findTestObject('Object Repository/kt session/Swagger Petstore/addPet')
def verifyResponse = WSBuiltInKeywords.sendRequest(verifyRequest)
WSBuiltInKeywords.verifyResponseStatusCode(verifyResponse, 405)
